#!/usr/bin/env python3
"""
Prefill 性能对比: sglang triton moe vs aiter moe (num_tokens=8192)
...
# 配置 (Qwen3-235B-A22B-FP8-dynamic with TP8, prefill)
num_tokens = 8192           # prefill with large batch
...
num_warmup = 10
num_runs = 100  # prefill 运行次数，捕获 100 次迭代，只下发一次

print("="*80)
print("MoE Prefill 性能对比测试 (CUDA Graph)")
print("="*80)
print(f"配置 (Qwen3-235B-A22B-FP8-dynamic with TP8, Prefill):")
print(f"  num_tokens={num_tokens} (large batch prefill)")
...
def prepare_inputs():
    ...
    # w1 shape: (num_experts, 2*inter_dim_per_rank, hidden_dim)
    # w2 shape: (num_experts, hidden_dim, inter_dim_per_rank)
    ...
    # scale shape: (num_experts, output_channels)
    # TP8: w1_scale 对应切分后的 inter_dim*2, w2_scale 对应 hidden_dim (不切分)
    ...
    # scale shape: (num_experts, output_channels, 1)
...
def benchmark_aiter_moe_with_cudagraph():
    ...
    print("\n" + "="*80)
    print("测试 1: aiter MoE (2-stage kernel) with CUDA Graph")
    print("="*80)
    ...
    print(f"普通模式平均耗时: {avg_time_normal_ms:.3f} ms")
    ...
    print(f"CUDA Graph 模式平均耗时: {avg_time_graph_ms:.3f} ms (单次迭代)")
...
def benchmark_sglang_triton_moe_with_cudagraph():
    ...
    print("\n" + "="*80)
    print("测试 2: sglang Triton MoE (per-channel quantization) with CUDA Graph")
    print("="*80)
    ...
    for _ in range(num_warmup):
        output = fused_moe(
            ...
            moe_config,
            use_fp8_w8a8=True,
            per_channel_quant=True,  # 使用 per-channel quantization
            w1_scale=w1_scale,       # sglang 使用 2D scale
            w2_scale=w2_scale,
        )
    torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("\n【普通模式 (含 kernel 启动开销)】")
    print(f"aiter MoE:          {aiter_time_normal:.3f} ms")
    print(f"sglang Triton MoE:  {triton_time_normal:.3f} ms")
    ...
    diff_percent = abs(aiter_time_normal - triton_time_normal) / max(aiter_time_normal, triton_time_normal) * 100
    print(f"性能差异: {diff_percent:.2f}%")
    
    print("\n【CUDA Graph 模式 (纯计算性能，消除下发开销)】")
    print(f"aiter MoE:          {aiter_time_graph:.3f} ms")
    print(f"sglang Triton MoE:  {triton_time_graph:.3f} ms")
    ...
    diff_percent = abs(aiter_time_graph - triton_time_graph) / max(aiter_time_graph, triton_time_graph) * 100
    print(f"性能差异: {diff_percent:.2f}%")
