"""
Performance benchmark: STA CUDA vs FA3 vs SDPA on NVIDIA H100.

H100 is SM 9.0, so st_attn CUDA kernel works natively.
FA3 is the primary baseline (as used in the STA paper).

FLOPs formula: 4 * B * H * N^2 * d (Q@K^T + P@V matmuls)
For sparse STA: effective FLOPs = dense FLOPs * (1 - sparsity)

Usage:
    CUDA_VISIBLE_DEVICES=0 python tests/bench_sta_h100.py [--warmup 5] [--repeat 20] [--output results.json]
"""
...
try:
    ...
    print(f"WARNING: st_attn CUDA kernel not available: {e}")
...
def run_benchmark(warmup=5, repeat=20):
    ...
    for shape_str, cfg in SHAPE_CONFIGS.items():
        ...
        print(f"{'=' * 120}")
        print(f"  {'Method':<40} {'Latency':>10} {'Speedup':>10} {'Memory':>10} {'TFLOPS':>8} {'L2 Err':>10} {'CosSim':>10} {'Sparsity':>9}")
        print(f"  {'─' * 115}")
        ...
        results.append({**base, "backend": "SDPA", "window": "full",
                        ...
                        "speedup_vs_sdpa": 1.0, "speedup_vs_fa3": None,
                        "tflops": round(sdpa_tflops, 1),
                        "dense_tflop": round(dense_tflop, 2)})
        print(f"  {'SDPA (baseline)':<40} {sdpa_lat:>9.2f}ms {'1.00x':>10} {sdpa_mem:>9.0f}MB {sdpa_tflops:>7.0f}")
        ...
        if FA3_AVAILABLE:
            ...
            try:
                ...
                results.append({**base, "backend": "FA3", "window": "full",
                                ...
                                "speedup_vs_sdpa": round(fa3_sp_sdpa, 2),
                                "speedup_vs_fa3": 1.0,
                                "tflops": round(fa3_tflops, 1),
                                "l2_rel_error": round(fa3_m["l2_rel_error"], 6),
                                "cosine_sim": round(fa3_m["cosine_sim"], 6),
                                "dense_tflop": round(dense_tflop, 2)})
                print(f"  {'FA3 (paper baseline)':<40} {fa3_lat:>9.2f}ms {fa3_sp_sdpa:>9.2f}x {fa3_mem:>9.0f}MB {fa3_tflops:>7.0f} "
        ...
        if ST_ATTN_AVAILABLE:
            ...
            for name, window in window_configs:
                ...
                try:
                    ...
                    speedup_sdpa = sdpa_lat / lat
                    speedup_fa3 = fa3_lat / lat if fa3_lat else None
                    eff_tflop = dense_tflop * (1 - sparsity)
                    tflops = eff_tflop / (lat / 1000)

                    # Check correctness
                    is_valid = m["cosine_sim"] > 0.99 and torch.isfinite(out).all().item()
                    validity_str = "" if is_valid else " [INVALID]"

                    results.append({
                        **base, "backend": f"STA_CUDA", "window": str(window),
                        "window_name": name,
                        "latency_ms": round(lat, 2), "peak_memory_mb": round(mem, 1),
                        "speedup_vs_sdpa": round(speedup_sdpa, 2),
                        "speedup_vs_fa3": round(speedup_fa3, 2) if speedup_fa3 else None,
                    ...
                    fa3_sp_str = f"(FA3 {speedup_fa3:.2f}x)" if speedup_fa3 else ""
                    print(f"  {name:<40} {lat:>9.2f}ms {speedup_sdpa:>9.2f}x {mem:>9.0f}MB {tflops:>7.0f} "
                          f"{m['l2_rel_error']:>10.6f} {m['cosine_sim']:>10.6f} {sp_str:>9}{validity_str} {fa3_sp_str}")
                except Exception as e:
...
def print_summary(results):
    print(f"\n{'=' * 160}")
    print("SUMMARY — STA CUDA vs FA3 vs SDPA on H100")
    print(f"{'=' * 160}")
    print(f"{'Backend':<15} {'Window':<15} {'Shape':<12} {'SeqLen':>8} {'Latency':>10} {'vs SDPA':>8} {'vs FA3':>8} "
          f"{'Memory':>10} {'TFLOPS':>8} {'L2 Err':>10} {'CosSim':>10} {'Sparsity':>9} {'Valid':>6}")
    print(f"{'─' * 160}")
    for r in results:
        ...
        fa3_sp = f"{r['speedup_vs_fa3']:.2f}x" if r.get('speedup_vs_fa3') else "—"
        valid = "Y" if r.get('valid', True) else "N"
        print(f"{r['backend']:<15} {r['window']:<15} {r['shape']:<12} {r['seq_len']:>8} "
              f"{r['latency_ms']:>9.2f}ms {r.get('speedup_vs_sdpa', 1.0):>7.2f}x {fa3_sp:>8} "
              f"{r['peak_memory_mb']:>9.0f}MB {tfl:>8} {l2:>10} {cos:>10} {sp:>9} {valid:>6}")
    print(f"{'=' * 160}")


def main():
    parser = argparse.ArgumentParser(description="STA CUDA vs FA3 vs SDPA Benchmark on H100")
